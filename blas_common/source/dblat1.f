...
*           .. DROTG ..
            ...
            CALL DTEST1(SA,DATRUE(K),DATRUE(K),SFAC)
            CALL DTEST1(SB,DBTRUE(K),DBTRUE(K),SFAC)
            CALL DTEST1(SC,DC1(K),DC1(K),SFAC)
            CALL DTEST1(SS,DS1(K),DS1(K),SFAC)
         ELSE
...
*     .. External Subroutines ..
      EXTERNAL          ITEST1, DSCAL, DTEST, DTEST1
*     .. Intrinsic Functions ..
      INTRINSIC         MAX
*     .. Common blocks ..
      COMMON            /COMBLA/ICASE, N, INCX, INCY, MODE, PASS
...
*              .. DNRM2 ..
               STEMP(1) = DTRUE1(NP1)
               CALL DTEST1(DNRM2(N,SX,INCX),STEMP,STEMP,SFAC)
            ELSE IF (ICASE.EQ.8) THEN
*              .. DASUM ..
               STEMP(1) = DTRUE3(NP1)
               CALL DTEST1(DASUM(N,SX,INCX),STEMP,STEMP,SFAC)
...
*              .. DSCAL ..
   ...
   40          CONTINUE
               CALL DTEST(LEN,SX,STRUE,STRUE,SFAC)
...
*     .. External Functions ..
      ...
      EXTERNAL          DDOT
*     .. External Subroutines ..
      EXTERNAL          DAXPY, DCOPY, DSWAP, DTEST, DTEST1
*     .. Intrinsic Functions ..
      INTRINSIC         ABS, MIN
*     .. Common blocks ..
      COMMON            /COMBLA/ICASE, N, INCX, INCY, MODE, PASS
...
*
            IF (ICASE.EQ.1) THEN
*              .. DDOT ..
               CALL DTEST1(DDOT(N,SX,INCX,SY,INCY),DT7(KN,KI),SSIZE1(KN)
     +                     ,SFAC)
...
*              .. DAXPY ..
               ...
               DO 40 J = 1, LENY
                  STY(J) = DT8(J,KN,KI)
   40          CONTINUE
               CALL DTEST(LENY,SY,STY,SSIZE2(1,KSIZE),SFAC)
...
*              .. DCOPY ..
   ...
   60          CONTINUE
               ...
               CALL DTEST(LENY,SY,STY,SSIZE2(1,1),1.0D0)
...
*              .. DSWAP ..
               ...
               DO 80 I = 1, 7
                  ...
                  STY(I) = DT10Y(I,KN,KI)
   80          CONTINUE
               CALL DTEST(LENX,SX,STX,SSIZE2(1,1),1.0D0)
               CALL DTEST(LENY,SY,STY,SSIZE2(1,1),1.0D0)
            ELSE
...
*              .. DROT ..
   ...
   20          CONTINUE
               ...
               CALL DTEST(LENX,SX,STX,SSIZE2(1,KSIZE),SFAC)
               CALL DTEST(LENY,SY,STY,SSIZE2(1,KSIZE),SFAC)
            ELSE
...
*
  ...
  180    CONTINUE
         ...
         CALL DTEST(5,COPYX,MWPSTX,MWPSTX,SFAC)
         CALL DTEST(5,COPYY,MWPSTY,MWPSTY,SFAC)
  200 CONTINUE
      RETURN
      END
